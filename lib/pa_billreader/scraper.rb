class PaBillreader::Scraper
	...
	def self.scrape_bill_detail(branch, bill)
		...
		binding.pry
		bill_details[:memo_url] = doc.css("div.BillInfo-CosponMemo div.BillInfo-Section-Data a").attr('href').value unless doc.css("div.BillInfo-CosponMemo div.BillInfo-Section-Data a").attr('href').value.nil?
		bill_details[:full_text_url] = "http://www.legis.state.pa.us#{doc.css("table.DataTable.BillInfo-PNTable td[valign='middle'] a").attr('href').value}" unless doc.css("table.DataTable.BillInfo-PNTable td[valign='middle'] a").attr('href').value.nil?
